...
from autogen import ConversableAgent, GroupChat, GroupChatManager, Agent, UserProxyAgent
...
def main():
    ...
    # 机器学习专家
    zhang_teacher = ConversableAgent(
        name="张老师",
...
def free_speak_pattern(last_speaker: Agent, groupchat: GroupChat) -> Union[Agent, str, None]:
    #     发言意向收集
    speaker_list = []
    prompt = ("你正在参与一个角色扮演游戏。角色有{agentlist}。他们的角色描述如下:\n{agent_sys_list}。你的角色是{"
              "agent}。理解上面的对话，然后判断你是否需要作为下一个发言人发言，使讨论更加热烈有效。只返回True或False，不要提供原因。")
    # prompt = "只返回True。"
    agentlist = f"{[agent.name for agent in groupchat.agents]}"
    agent_sys_list = f"{"\n".join([agent.name + ':' + agent.description for agent in groupchat.agents])}"
    ...
    thread_list = [threading.Thread(
        ...
        kwargs={
            ...
            "agent": agent,
            "msgs": groupchat.messages.copy(),
            "prompt": prompt.format(agent=agent.name, agentlist=agentlist, agent_sys_list=agent_sys_list),
        }
...
def _free_speak_pattern_thread(speaker_list=None, agent=None, msgs=None, prompt=""):
    if speaker_list is None:
        speaker_list = []
    out_agent = ConversableAgent("out_agent",
                                 code_execution_config=False,
                                 human_input_mode="NEVER")
    judge_agent = ConversableAgent("judge_agent", llm_config=llm_config,
                                   chat_messages={out_agent: msgs},
                                   human_input_mode="NEVER")
    chat_result = out_agent.initiate_chat(
        judge_agent,
        cache=None,
        message={
            "content": prompt,
    ...
    result = chat_result.chat_history[-1].get("content", "False")
    if result.lower().find("true") > -1:
        with lock:
